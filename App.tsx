...
const App: React.FC = () => {
  ...
  const handleAnnotationConfirm = async (finalImage: string, loc: string) => {
    const cleanLoc = sanitizeInput(loc); 
  ...
  return (
    <div className="flex-grow flex flex-col bg-slate-50 text-slate-800 relative font-sans min-h-screen">
      ...
      <main className="flex-grow container mx-auto px-4 py-8 z-10 flex flex-col min-h-[calc(100vh-64px)] overflow-y-auto">
        {step === 'upload' && (
          <div className="flex-grow flex flex-col items-center justify-center space-y-8 animate-in fade-in zoom-in-95 duration-700 py-4">
        ...
        {step === 'annotate' && image && (
            <div className="flex-grow flex flex-col items-center justify-center max-w-6xl mx-auto w-full animate-in zoom-in-95 duration-500 py-4 min-h-[600px]">
                 <div className="w-full h-full bg-white border border-slate-200 rounded-lg shadow-lg overflow-hidden relative min-h-[500px]">
                    <ImageAnnotator 
                        imageBase64={image} 
        ...
        {(step === 'analyzing' || step === 'result') && annotatedImage && (
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 h-full min-h-0 py-4">
            ...
            <div className="lg:col-span-6 xl:col-span-7 flex flex-col min-h-[400px]">
            ...
            <div className="lg:col-span-6 xl:col-span-5 flex flex-col min-h-[500px]">
              {error ? (
                <div className="h-full border border-red-200 bg-red-50 rounded-lg p-8 flex flex-col items-center justify-center text-center relative shadow-md">
                  <button onClick={handleReset} className="absolute top-4 right-4 text-slate-400 hover:text-slate-600">
                    <X className="w-6 h-6" />
                  </button>
                 ...
                 <LoadingOverlay onCancel={handleReset} />
              )}
            </div>
          </div>
        )}
